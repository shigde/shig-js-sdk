...
export class ReqTokenInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
        const urlIsInList = this.isUrlInWhiteList(request.url, request.method);
    ...
    private isUrlInWhiteList(url: string, method: string): boolean {
        return url.endsWith('whep')
            || url.endsWith('live')
            || (url.endsWith('whip') && method === 'DELETE');
    }
}
